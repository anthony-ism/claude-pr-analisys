...
function getTestTicketId() {
    if (!process.env.JIRA_TICKET_PREFIX) {
        throw new Error('Test environment not properly set up. JIRA_TICKET_PREFIX must be set.');
    }
...
function validateTestEnvironment(requiredVars = ['JIRA_TICKET_PREFIX']) {
    ...
    if (missing.length > 0) {
        throw new Error(`Test environment missing required variables: ${missing.join(', ')}`);
    }
}
 ...
 */
function cleanupTestEnvironment(varsToClean = ['JIRA_TICKET_PREFIX', 'GITHUB_REPOSITORY']) {

...
function getClaudeResponseScenarios() {
    ...
    return {
        ...
        withConcerns: generateMockAnalysis(ticketId).replace('85%', '65%').replace('High confidence', 'Moderate confidence'),
        needsWork: generateMockAnalysis(ticketId).replace('✅', '⚠️').replace('85%', '45%'),
        comprehensive: generateMockAnalysis(ticketId) + '\n\n## Additional Analysis\n\nDetailed code review findings...'
    };
}
...
function isValidClaudeCommand(command) {
    return Object.values(claudeCommandPatterns).some(pattern => pattern.test(command));
}

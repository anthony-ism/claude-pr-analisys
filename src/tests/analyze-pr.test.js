/**
 * analyze-pr.test.js - Tests for analyze-pr.js script
...
const analyzePr = require('../analyze-pr');
...
testRunner.test('Create Claude prompt with all required data', async () => {
    setupMocks();
    
    const prompt = await analyzePr.createClaudePrompt('393', testTicketId, mockPRData, mockJiraData);
    
    assertTruthy(prompt, 'Should create prompt');
    assertContains([prompt], 'Pull Request #393', 'Should include PR number');
    assertContains([prompt], testTicketId, `Should include Jira ticket ${testTicketId}`);
    assertContains([prompt], 'stopFormValidation', 'Should include diff content');
...
testRunner.test('Call Claude function creates timestamped file', async () => {
    ...
    if (fs.existsSync(tempDir)) {
        ...
        assert(promptFiles.length > 0, 'Should create prompt file in temp directory');
...
testRunner.test('Usage function provides help text', async () => {
    // Test that usage function exists and is callable
    assertTruthy(typeof analyzePr.usage === 'function', 'Usage should be a function');
...
testRunner.test('Module exports required functions', async () => {
    assertTruthy(typeof analyzePr.main === 'function', 'Should export main function');
    assertTruthy(typeof analyzePr.createClaudePrompt === 'function', 'Should export createClaudePrompt');
    assertTruthy(typeof analyzePr.callClaude === 'function', 'Should export callClaude');
    assertTruthy(typeof analyzePr.usage === 'function', 'Should export usage');
});

// Test 5: Integration with shared utilities
testRunner.test('Integration with shared utilities', async () => {
    setupMocks();
    ...
    assert(jiraTicket === testTicketId, `Should use shared Jira extraction and extract ${testTicketId}`);
    
    restoreMocks();
});

// Test 6: Error handling in prompt creation
testRunner.test('Error handling in prompt creation', async () => {
    setupMocks();
    
    // Test with invalid data
    try {
        const prompt = await analyzePr.createClaudePrompt('393', testTicketId, null, mockJiraData);
        // Should handle null prData gracefully
        assertTruthy(prompt, 'Should create prompt even with null prData');
    } catch (error) {
        // If it throws, that's also acceptable behavior
        assertTruthy(error.message, 'Should provide meaningful error message');
    }
...
testRunner.test('File creation in correct temp location', async () => {
    const fs = require('fs');
    const path = require('path');
    ...
    assert(afterFiles.length > beforeFiles.length, 'Should create new file in temp directory');
...
testRunner.test('Prompt template includes all required sections', async () => {
    setupMocks();
    ...
    assertContains([prompt], '### PR Changes (diff):', 'Should have diff section');
    assertContains([prompt], '### PR Metadata:', 'Should have metadata section');
    assertContains([prompt], '## Jira Ticket', 'Should have Jira section');
    assertContains([prompt], '### Ticket Details:', 'Should have ticket details');
    assertContains([prompt], '## Analysis Request', 'Should have analysis request');
    assertContains([prompt], 'IMPORTANT: Before analyzing', 'Should have context gathering instructions');
    assertContains([prompt], 'Context Summary', 'Should request context summary');
    assertContains([prompt], 'Confidence Level', 'Should request confidence level');

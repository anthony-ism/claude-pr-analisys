...
import { getTestTicketId } from '../testing/mocks';
...
describe('PR Utils Test Suite', () => {
  ...
  test('Extract Jira ticket from PR title', async () => {
    const ticket = prUtils.extractJiraTicket(
  ...
  test('GitHub pattern matching', async () => {
    ...
    expect(
      Array.isArray(patterns.valid) || typeof patterns.valid === 'object'
    ).toBe(true);
  });
  ...
  test('Jira CLI command construction', async () => {
    ...
    const calls = mockExecutor.getCalls();
    expect(
      calls.some(call => call.includes(`jira issue view ${testTicketId}`))
    ).toBe(true);
  });
  ...
  test('Ticket extraction with various formats', async () => {
    const testCases = [
      ...
      {
        input: `feat: ${testTicketId} - Complex feature`,
        expected: testTicketId,
      },
      { input: 'No ticket in this title', expected: null },
      {
        input: `Multiple ${testTicketId} and TEST-999`,
        expected: testTicketId,
      },
    ];

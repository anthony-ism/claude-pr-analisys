 ...
 * @returns {string} Mock Claude prompt
 */
function getMockClaudePrompt(prNumber, ticketId, prData, jiraData) {
...
- URL: ${prData ? prData.json.url : `https://github.com/test-org/test-repo/pull/${prNumber}`}
 ...
 */
function getMockClaudeResponse(ticketId) {
    const ticket = ticketId || getTestTicketId();
 ...
 */
function getMockPromptVariations() {
    const ticketId = getTestTicketId();
    return {
        bugFix: getMockClaudePrompt('123', ticketId, null, 'Bug: Form validation issue'),
        feature: getMockClaudePrompt('124', `${ticketId.split('-')[0]}-1234`, null, 'Feature: New functionality'),
        refactor: getMockClaudePrompt('125', `${ticketId.split('-')[0]}-5678`, null, 'Refactor: Code cleanup')
    };
}

/**
 * pr-utils.test.js - Unit tests for shared PR utilities
 * Author: Anthony Rizzo, Co-pilot: Claude
 * Description: Comprehensive tests for pr-utils.js functionality
 */
...
testRunner.test('Console utilities - colors and printStatus', async () => {
    ...
    assertTruthy(typeof prUtils.printStatus === 'function', 'printStatus should be a function');
...
testRunner.test('PR number validation', async () => {
    // Valid PR numbers
...
testRunner.test('PR validation with GitHub CLI', async () => {
    setupMocks();
...
testRunner.test('PR validation failure handling', async () => {
    setupMocks();
    
    // Clear default patterns and set specific error case
    mockExecutor.regexResponses.clear();
    mockExecutor.responses.clear();
...
testRunner.test('Gather PR data from GitHub', async () => {
    setupMocks();
    ...
    assert(prData.json.title.includes(testTicketId), `Should include correct ticket ID ${testTicketId} in title`);
    
    // Check that all required commands were called (flexible matching)
    const calls = mockExecutor.getCalls();
    const foundBasicView = calls.some(call => call.includes('gh pr view 393') && !call.includes('--json'));
...
testRunner.test('File utilities - ensure temp directory', async () => {
...
testRunner.test('File utilities - create timestamped file', async () => {
...
testRunner.test('File validation', async () => {
    // Create test file
...
testRunner.test('Tool checking functionality', async () => {
    setupMocks();
...
testRunner.test('Tool checking failure handling', async () => {
    setupMocks();
    
    // Mock missing tool
    mockExecutor.setResponse('which missing-tool', new Error('Command not found'));
...
testRunner.test('Jira ticket extraction from PR titles', async () => {
    // Test valid ticket extraction
    const ticket1 = prUtils.extractJiraTicket(`${testTicketId}: Unable to set forms to Ready to Read`);
    assertEqual(ticket1, testTicketId, 'Should extract ticket ID');
    
    const ticket2 = prUtils.extractJiraTicket(`Fix: ${testTicketId.replace('-2055', '-1234')} - Update validation logic`);
    assertEqual(ticket2, testTicketId.replace('-2055', '-1234'), 'Should extract ticket from middle of title');
    
    // Test no ticket found
    const noTicket = prUtils.extractJiraTicket('Fix bug without ticket reference');
    assertEqual(noTicket, null, 'Should return null for no ticket');
});

// Test 12: Jira ticket validation
testRunner.test('Jira ticket validation', async () => {
    setupMocks();
    ...
    const calls = mockExecutor.getCalls();
    const foundJiraCall = calls.some(call => call.includes(`jira issue view ${testTicketId}`));
...
testRunner.test('Jira data gathering', async () => {
    setupMocks();
...
testRunner.test('Claude model detection', async () => {
    setupMocks();
...
testRunner.test('PR comment posting', async () => {
    setupMocks();
    ...
    const calls = mockExecutor.getCalls();
    const foundCommentCall = calls.some(call => call.includes('gh pr comment 393'));
...
testRunner.test('Error handling in PR data gathering', async () => {
    setupMocks();
...
testRunner.test('Error handling in Jira data gathering', async () => {
    setupMocks();
    
    // Mock error in Jira view
    mockExecutor.setResponse('jira issue view INVALID', new Error('Ticket not found'));
...
testRunner.test('Empty file validation', async () => {
...
testRunner.test('Large file creation and validation', async () => {
    const largeContent = 'x'.repeat(1000);
    const largeFile = prUtils.createTimestampedFile('large-test', '.txt', largeContent);
...
testRunner.test('Integration test - full workflow simulation', async () => {
    setupMocks();
